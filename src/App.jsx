...
function App() {
  const [inventory, setInventory] = useState(() => {
    const saved = localStorage.getItem('office_inventory_single_file');
    if (saved) {
  ...
  useEffect(() => {
    localStorage.setItem('office_inventory_single_file', JSON.stringify(inventory));
  }, [inventory]);
  ...
  return (
    <div style={inlineStyles.pageWrapper} dir="rtl">
      {/* עיצוב פנימי מובנה שאינו תלוי בקבצים חיצוניים */}
      <style>{`
        body {
          margin: 0 !important;
          padding: 0 !important;
